        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                ...
                if (result.Succeeded)
                {
                    ...
                    // Upewnij się, że rola "Reader" istnieje
                    if (!await _roleManager.RoleExistsAsync("Reader"))
                    {
                        var roleResult = await _roleManager.CreateAsync(new IdentityRole("Reader"));
                        if (!roleResult.Succeeded)
                        {
                            ModelState.AddModelError(string.Empty, "Failed to create Reader role.");
                            return Page();
                        }
                    }

                    // Przypisanie roli "Reader" do użytkownika
                    var roleAssignResult = await _userManager.AddToRoleAsync(user, "Reader");
                    ...
                    // Aktualizacja pola "Role" w tabeli AspNetUsers
                    user.Role = "Reader";
